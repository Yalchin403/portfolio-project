    ...
    def get(self, request):
        ...
        if request.GET.get("search"):
            ...
            for search in search_list:
                objs = (
                    Blog.objects.order_by("-time_pub")
                    .filter(Q(title__icontains=search) | Q(descript__icontains=search))
                    .distinct()
    ...
    if request.method == "POST" and request.user.is_authenticated:
        if content:
            ...
            if not already_exists:
                owner = request.user
                reply_obj = Comment(
                    content=content, owner=owner, parent=comment_obj, blog=blog_obj
                )
                reply_obj.save()
        form = CommentForm()
        comment_qs = blog_obj.comments.all()
...
def up_vote(request, pk):
    ...
    if comment_obj and user_obj:
        if user_obj in comment_obj.up_votes.all():
            ...
            data = {"up_voted": False, "net_votes": net_votes}
        ...
        net_votes = comment_obj.net_votes
        data = {"up_voted": True, "net_votes": net_votes}
        return JsonResponse(data)
...
def down_vote(request, pk):
    ...
    if comment_obj and user_obj:
        if user_obj in comment_obj.down_votes.all():
            comment_obj.down_votes.remove(user_obj)
            net_votes = comment_obj.net_votes
            data = {"down_voted": False, "net_votes": net_votes}
            return JsonResponse(data)
        ...
        net_votes = comment_obj.net_votes
        data = {"down_voted": True, "net_votes": net_votes}
        return JsonResponse(data)
