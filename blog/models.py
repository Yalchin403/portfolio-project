...
class Subscription(models.Model):
    ...
    def save(self, *args, **kwargs):
        try:
            email_body = (
                f"Congrats, you've got new subscriber<br>Name: {self.full_name}"
            )
            email = "yalchinmammadli@yalchin.info"

            msg = EmailMessage("New Subscription", email_body, EMAIL_HOST_USER, [email])
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.boss_notified:
            try:
                ...
                msg = EmailMessage(
                    f"New Comment by {self.owner}", email_body, EMAIL_HOST_USER, [email]
                )
                msg.content_subtype = "html"
                msg.send()
        ...
        if self.parent and self.is_verified:
            ...
            try:
                ...
                msg = EmailMessage(
                    f"New Reply by {self.owner}", email_body, EMAIL_HOST_USER, [email]
                )
                msg.content_subtype = "html"
                msg.send()
            except:
...
def send_notification_2_subs(post_id, subject):
    ...
    for sub_obj in subs_objs:
        ...
        try:
            ...
            msg = EmailMessage(subject, email_body, EMAIL_HOST_USER, [email])
            msg.content_subtype = "html"
            msg.send()
